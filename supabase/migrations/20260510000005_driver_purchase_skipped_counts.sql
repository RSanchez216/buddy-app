-- Adds two count columns to v_driver_purchase_summary:
--   • skipped_count_total — every skipped period ever
--   • skipped_count_recent — skips in the last 90 days (the recency
--     window that determines whether the list pill renders)
-- Pure additions to the view's projection; everything else stays byte-
-- identical. CREATE OR REPLACE is fine since we're not removing columns.
...
SELECT
  ...
  -- Skipped surfaces — counts only, not a join. recent uses skipped_at
  -- (when the user marked it skipped) rather than period_end so a
  -- skip recorded today against a 6-month-old period still surfaces.
  (SELECT count(*)::int FROM driver_purchase_payments p
