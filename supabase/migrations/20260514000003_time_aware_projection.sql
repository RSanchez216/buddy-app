-- Time-aware projection: balance entries no longer dictate strict order-of-
-- operations. paid_at + transfer.created_at let projected_balances() decide,
-- on the anchor day, whether each event was recorded BEFORE the balance
-- (already baked in → exclude) or AFTER (not yet baked in → include).
--
-- The Slice 3 variance trigger continues to fire only on balance entry
-- changes. Out-of-order edits to historical paid_at / transfer.created_at
-- may still need manual cleanup.

ALTER TABLE public.custom_outflows ADD COLUMN IF NOT EXISTS paid_at TIMESTAMPTZ;
ALTER TABLE public.invoices       ADD COLUMN IF NOT EXISTS paid_at TIMESTAMPTZ;

-- Backfill. custom_outflows has updated_at; invoices does not, so we cast
-- paid_date to a midnight timestamp — the best historical approximation for
-- rows that pre-date the paid_at concept. Rows without paid_date stay NULL
-- and get filtered out of the time-aware branch (they're already excluded
-- by the paid_date = ds.d join condition anyway).
UPDATE public.custom_outflows SET paid_at = updated_at
WHERE status = 'paid' AND paid_at IS NULL;

UPDATE public.invoices SET paid_at = paid_date::timestamptz
WHERE status = 'Paid' AND paid_at IS NULL AND paid_date IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_co_paid_at
  ON public.custom_outflows (funding_account_id, paid_date, paid_at)
  WHERE status = 'paid';

CREATE INDEX IF NOT EXISTS idx_inv_paid_at
  ON public.invoices (funding_account_id, paid_date, paid_at)
  WHERE status = 'Paid';

-- projected_balances v4 — time-aware on anchor day for outflows + transfers.
-- Non-anchor days keep the existing planned-only behaviour.
