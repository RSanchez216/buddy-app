-- Phase 3 polish — title_release_pending derived flag.
-- A "Fully Paid" contract still owes the driver a physical title hand-off
-- (title_transferred=false). This flag drives a list-page alert panel,
-- a filter chip, a row badge, and the "Mark title as transferred" detail-
-- page action.
--
-- Day-1 baseline: ~45 contracts come back true because the ClickUp import
-- defaulted title_transferred=false for every record. The alert panel
-- exists exactly so Rebeca can audit and mark them as the physical hand-
-- off is confirmed.
...
SELECT
  ...
  dp.total_value, dp.downpayment, dp.sale_price, dp.current_balance,
