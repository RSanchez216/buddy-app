      ...
      {days.map((day, i) => {
        ...
        return (
          ...
          >
            ...
            {/* Day footer — totals + by-bank breakdown + shortfall pill */}
...
// Group chips under bank section headers, sorted by |net| desc; Unassigned pinned to bottom.
...
function DayFooter({ bucket, shortfall }) {
  ...
  // Build bank breakdown sorted by |net| desc; Unassigned pinned to bottom and only shown if it has events
  const banks = Object.entries(bucket.banks)
    .map(([key, b]) => ({ key, ...b, net: b.inflow - b.outflow }))
