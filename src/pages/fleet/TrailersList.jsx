...
import { StagePill, OWNERSHIP_STAGES, trailerTypePillClasses, inspectionTone, fmtDate } from './fleetUtils'
...
export default function TrailersList() {
  ...
  const filtered = useMemo(() => {
    ...
    const searched = q ? base.filter(r =>
      ...
      (r.trailer_type || '').toLowerCase().includes(q) ||
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Trailers</h1>
        ...
        {canEdit && (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="Upload the TMS Trailer Assignments export to refresh assignment history + current drivers"
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <input
          ...
          placeholder="Search unit, VIN, type, plate, owner, driver…"
          value={filter}
          ...
          <table className="w-full text-sm">
            <thead className={S.tableHead}>
              <tr>
                ...
                <th className={`${S.th} min-w-[120px]`}>Type</th>
                <th className={`${S.th} min-w-[140px]`}>Inspection Exp.</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400 dark:text-slate-600 text-sm">
                  {rows.length === 0
                    ? "No trailers yet. Click 'Upload Excel' to import a TMS export or 'Add Trailer' to add one manually."
                ...
                return (
                  <tr key={r.id} className={S.tableRow}>
                    ...
                    <td className={S.td}>
                      {r.trailer_type ? (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${trailerTypePillClasses(r.trailer_type)}`}>
                          {r.trailer_type}
                        </span>
                      ) : <span className="text-gray-400 dark:text-slate-600">—</span>}
                    </td>
                    <td className={`${S.td} whitespace-nowrap`}>
                      {r.annual_inspection_expiration_date ? (
                        <div className="flex items-center gap-1.5">
                          <span className={`w-1.5 h-1.5 rounded-full ${tone.dot}`} />
                          <span className={`text-xs ${tone.text}`}>{fmtDate(r.annual_inspection_expiration_date)}</span>
                    ...
                    <td className={`${S.td} text-right whitespace-nowrap`}>
                      ...
                      <Link to={`/fleet/trailers/${r.id}`} className="text-gray-400 hover:text-orange-600 dark:hover:text-orange-400" title="View detail">
