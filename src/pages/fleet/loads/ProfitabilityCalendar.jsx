    ...
    <div className="space-y-3">
      ...
      <div className={`${S.card} max-h-[70vh] overflow-auto`}>
        <div className="inline-block min-w-full" style={{ display: 'grid', gridTemplateColumns: '180px repeat(7, minmax(80px, 1fr)) 200px', gap: '0.5px', backgroundColor: '#f3f4f6', padding: '0.5px' }}>
          ...
          {/* Driver rows */}
          {filtered.map((driver, driverIndex) => {
            ...
            const rowBgLight = isEvenRow ? 'bg-slate-50 dark:bg-transparent' : 'bg-white dark:bg-slate-900'
            const rowBgDark = isEvenRow ? 'dark:bg-white/[0.04]' : 'dark:bg-transparent'
            return (
              <div key={driver.key} className="contents">
                {/* Driver cell (sticky left) */}
                <div className={`sticky left-0 z-10 ${rowBgLight} ${rowBgDark} border-b border-slate-300 dark:border-slate-700 px-3 py-3 text-xs font-medium text-gray-900 dark:text-slate-200 min-h-[80px]`} style={{ minWidth: '180px' }}>
                ...
                <div className={`col-span-7 ${rowBgLight} ${rowBgDark} border-b border-slate-300 dark:border-slate-700 p-0.5 relative`} style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 1, minHeight: `${36 * (maxTrack + 1) + 8}px` }}>
                ...
                <div className={`sticky right-0 z-10 ${rowBgLight} ${rowBgDark} border-b border-slate-300 dark:border-slate-700 px-3 py-3 text-[11px] text-gray-600 dark:text-slate-400 min-h-[80px] grid grid-cols-2 gap-2`} style={{ minWidth: '200px' }}>
