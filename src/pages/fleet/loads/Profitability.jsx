...
export default function Profitability() {
  ...
  const load = useCallback(async () => {
    ...
    const [rollup, prior, team] = await Promise.all([
      supabase.rpc('load_profit_rollup', { p_dimension: dimension, p_from: range.from, p_to: range.to, p_basis: basis }),
      supabase.rpc('load_profit_rollup', { p_dimension: dimension, p_from: priorFrom, p_to: priorTo, p_basis: basis }),
      loadTeamLoads(),
    ])
    ...
    setTeamLoads(team)
    setLoading(false)
  }, [dimension, range.from, range.to, toast, basis])
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col gap-2 lg:items-end">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* By delivery | By pickup toggle */}
            <div className="flex rounded-lg overflow-hidden border border-gray-200 dark:border-slate-700 text-xs shrink-0">
              <button
                onClick={() => setBasis('delivery')}
                className={`px-3 py-1.5 whitespace-nowrap shrink-0 ${basis === 'delivery' ? 'bg-orange-500 text-slate-900 font-semibold' : 'text-gray-500 dark:text-slate-400'}`}
              >
                By delivery
              </button>
              <button
                onClick={() => setBasis('pickup')}
                className={`px-3 py-1.5 whitespace-nowrap shrink-0 ${basis === 'pickup' ? 'bg-orange-500 text-slate-900 font-semibold' : 'text-gray-500 dark:text-slate-400'}`}
              >
                By pickup
              </button>
            </div>
          ...
          <p className="text-[11px] text-gray-400 dark:text-slate-500">
            {PRESET_LABEL[preset]} · <span className="font-medium text-gray-500 dark:text-slate-400">{formatRange(range.from, range.to)}</span> · by {basis} date
          </p>
        </div>
      </div>
