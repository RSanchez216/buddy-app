...
// is what a broker will argue about — not when the request was typed.
...
export default function SummaryBand({ summary, loading }) {
  ...
  const cards = [
    { label: 'Requested', amount: s.claimed_amount,   count: s.claimed_count,   tone: 'plain' },
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
        ...
        <div className="sm:col-span-2 rounded-2xl border-2 border-orange-300 dark:border-orange-500/40 bg-gradient-to-br from-orange-50 to-orange-100/60 dark:from-orange-500/[0.12] dark:to-orange-500/[0.04] p-4">
          ...
          <p className="text-[11px] text-orange-700/70 dark:text-orange-400/70">{Number(s.awaiting_count) || 0} request{Number(s.awaiting_count) === 1 ? '' : 's'} outstanding</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        {/* Filing performance — how promptly requests are being raised */}
        <div className={'rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0d0d1f] px-4 py-3'}>
          ...
          <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">Measured from the load&apos;s delivery date. Requests filed before delivery are excluded from the average.</p>
        </div>
      </div>
    </div>
  )
...
function Card({ label, amount, count, tone }) {
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0d0d1f] px-4 py-3">
      ...
      <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">{Number(count) || 0} request{Number(count) === 1 ? '' : 's'}</p>
    </div>
  )
}
