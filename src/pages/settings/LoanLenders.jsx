...
export default function SettingsLoanLenders() {
  ...
  async function load() {
    const { data } = await supabase.from('loan_lenders').select('*').order('name')
  ...
  async function save() {
    ...
    const payload = {
      name: form.name.trim(),
      paynet_email: form.paynet_email.trim() || null,
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Loan Lenders</h1>
          <p className="text-sm text-gray-500 dark:text-slate-500 mt-0.5">Banks and finance companies that hold loans</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead className={S.tableHead}>
            <tr>
              ...
              <th className={S.th}>Paynet Email</th>
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400 dark:text-slate-600 text-sm">No lenders yet</td></tr>
            ) : items.map(it => (
              <tr key={it.id} className={S.tableRow}>
                <td className={`${S.td} font-medium text-gray-900 dark:text-slate-200`}>{it.name}</td>
                <td className={`${S.td} text-gray-500 dark:text-slate-400 font-mono text-xs`}>{it.paynet_email || '—'}</td>
                <td className={`${S.td} text-gray-500 dark:text-slate-400 text-xs max-w-xs truncate`} title={it.contact_info || ''}>{it.contact_info || '—'}</td>
                <td className={S.td}>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Lender' : 'Add Lender'}>
        <div className={S.modalBody}>
          ...
          <div>
            <label className={S.label}>Paynet Email</label>
            <input className={S.input} value={form.paynet_email} onChange={e => setForm(f => ({ ...f, paynet_email: e.target.value }))} placeholder="contact@lender.com" />
          </div>
          <div>
            <label className={S.label}>Contact Info</label>
            <input className={S.input} value={form.contact_info} onChange={e => setForm(f => ({ ...f, contact_info: e.target.value }))} placeholder="Phone / address / rep name" />
          </div>
          <div>
