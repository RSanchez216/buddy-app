...
export default function SettingsLoanEntities() {
  ...
  async function load() {
    const { data } = await supabase.from('loan_entities').select('*').order('name')
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Loan Entities</h1>
          <p className="text-sm text-gray-500 dark:text-slate-500 mt-0.5">Borrowing entities (LLCs, etc.) used in Debt Schedule</p>
        </div>
      ...
      <div className={`${S.card} overflow-hidden`}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={3} className="px-4 py-12 text-center text-gray-400 dark:text-slate-600 text-sm">No entities yet</td></tr>
            ) : items.map(it => (
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Entity' : 'Add Entity'} size="sm">
        <div className={S.modalBody}>
          {error && <div className={S.errorBox}>{error}</div>}
          <div>
            <label className={S.label}>Name *</label>
            <input className={S.input} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Manas Express LLC" />
          </div>
